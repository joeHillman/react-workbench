...
class TabContainer extends Component {
  ...
  render() {
    ...
    const panelContent = () => thisMenu(menuObj).contentTabs[this.state.activeTab - 1].content

    return (
      <div className="c-tab-container">
        <TabRender>{thisMenu(menuObj).theseTabs}</TabRender>
        <div className="c-tab-content">{panelContent()}</div>
      </div>
    );
  }
}
